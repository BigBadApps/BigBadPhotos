...
def test_score_exposure_perfect():
    ...
    assert res['mean_brightness'] == 117.3
    assert res['exposure_score'] == 1.0

def test_score_exposure_overexposed():
    # Mean > 118, highlighting clippings
    gray = np.full((10, 10), 255, dtype=np.uint8)
    res = score_exposure(gray)
    assert res['mean_brightness'] == 255.0
    assert res['highlight_clip_pct'] == 100.0
    assert res['shadow_clip_pct'] == 0.0
...
def test_score_exposure_underexposed():
    ...
    assert res['highlight_clip_pct'] == 0.0
    assert res['shadow_clip_pct'] == 100.0
    assert res['exposure_score'] == 0.0
...
def test_score_exposure_mixed():
    ...
    gray[:5, :] = 255
    res = score_exposure(gray)
    assert res['mean_brightness'] == 127.5
    assert res['highlight_clip_pct'] == 50.0
    assert res['shadow_clip_pct'] == 50.0
    assert res['exposure_score'] == 0.428
